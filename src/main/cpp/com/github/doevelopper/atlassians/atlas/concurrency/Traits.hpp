...
*/
#ifndef COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_TRAITS_H
#define COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_TRAITS_H
...
// #define UNUSED(x) (void)(x) // remove compiler warnings
        ...
        struct Traits
        {
            ...
            template <class IT>
            using IsInputIterator = std::enable_if_t<std::is_convertible<
                typename std::iterator_traits<IT>::iterator_category, std::input_iterator_tag>::value>;
            ...
            struct DerivedFrom
            {
                ...
                operator B *()
                {
                    return static_cast<D *>(static_cast<THIS *>(this));
                }
        ...
        template <class T>
        struct Traits::InnerType<std::vector<T>>
        {
            using Type = T;
        };
        template <class T, class V>
        using BufferType = std::enable_if_t<
            Traits::IsBuffer<T>::value && !std::is_same<std::decay_t<V>, T>::value
            && std::is_convertible<std::decay_t<V>, typename Traits::IsBuffer<T>::Type>::value>;
        template <class T, class V>
        using NonBufferType =
            std::enable_if_t<!Traits::IsBuffer<T>::value && std::is_convertible<std::decay_t<V>, T>::value>;
        template <class T>
        ...
        template <typename FUNC>
        auto coroResult(FUNC && func) -> typename std::conditional<
            Traits::IsVoidContext<decltype(firstArgOf(func))>::value, decltype(resultOf2(func)),
            decltype(resultOf(func))>::type;
        template <typename FUNC>
        auto ioResult(FUNC && func) -> typename std::conditional<
            Traits::IsThreadPromise<decltype(firstArgOf(func))>::value, decltype(resultOf(func)),
            decltype(resultOf2(func))>::type;
        template <typename FUNC>
        auto mappedKeyOf(FUNC && func) ->
            typename std::tuple_element<0, typename Traits::InnerType<decltype(resultOf2(func))>::Type>::type;
        template <typename FUNC>
        auto mappedTypeOf(FUNC && func) ->
            typename std::tuple_element<1, typename Traits::InnerType<decltype(resultOf2(func))>::Type>::type;
        template <typename FUNC>
...
#endif // COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_TRAITS_H
