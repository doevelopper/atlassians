...
{
        inline void SpinLockUtil::pauseCPU()
        {
#if defined(_MSC_VER) && (defined(_M_IX86) || defined(_M_IA64) || defined(_M_X64))
            ::_mm_pause();
#elif (defined(__GNUC__) || defined(__llvm__)) && (defined(__i386__) || defined(__ia64__) || defined(__x86_64__))
        ...
        inline bool SpinLockUtil::lockWrite(std::atomic_uint32_t & flag, LockTraits::Attempt attempt)
        {
        ...
        inline bool SpinLockUtil::upgradeToWrite(std::atomic_uint32_t & flag, LockTraits::Attempt attempt)
        {
            bool pendingUpgrade = false;
            return upgradeToWriteImpl(flag, pendingUpgrade, attempt);
        }

        inline bool SpinLockUtil::upgradeToWrite(
            std::atomic_uint32_t & flag, bool & pendingUpgrade, LockTraits::Attempt attempt)
        {
            return upgradeToWriteImpl(flag, pendingUpgrade, attempt);
        }

        inline bool SpinLockUtil::upgradeToWriteImpl(
            std::atomic_uint32_t & flag, bool & pendingUpgrade, LockTraits::Attempt attempt)
        {
            size_t numBackoffs = 0;
        spin:
        ...
        inline bool SpinLockUtil::lockRead(std::atomic_uint32_t & flag, LockTraits::Attempt attempt)
        {
            size_t numBackoffs = 0;
        spin:
            if (attempt == LockTraits::Attempt::Unlimited)
        ...
        inline void SpinLockUtil::unlockRead(std::atomic_uint32_t & flag)
        {
            uint32_t oldValue = set(0, 1);
        ...
        inline void SpinLockUtil::unlockWrite(std::atomic_uint32_t & flag)
        {
            uint32_t oldValue = set(0, -1);
            uint32_t newValue = set(0, 0);
            while (!flag.compare_exchange_weak(oldValue, newValue, std::memory_order_acq_rel))
        ...
        inline void SpinLockUtil::yieldOrSleep(size_t & num)
        {
        ...
        inline size_t SpinLockUtil::generateBackoff()
        {
                ...
                || (SpinLockTraits::backoffPolicy() == SpinLockTraits::BackoffPolicy::Random))
            {
                ...
                return distribution(
                    gen, Distribution::param_type {SpinLockTraits::minSpins(), SpinLockTraits::maxSpins()});
            }
            else
            {
                // Generate a number below min and add it to the min.
                return SpinLockTraits::minSpins()
                     + distribution(gen, Distribution::param_type {0, SpinLockTraits::minSpins()});
            }
        }

        inline void SpinLockUtil::backoff(size_t & num)
        {
            if (num == 0)
            {
                num = generateBackoff();
        ...
        inline void SpinLockUtil::spinWaitWriter(std::atomic_uint32_t & flag)
        {
            size_t numIters  = 0;
            size_t numYields = 0;
        ...
        inline void SpinLockUtil::spinWaitReader(std::atomic_uint32_t & flag)
        {
            size_t numIters  = 0;
            size_t numYields = 0;
            while (true)
        ...
        inline bool SpinLockUtil::isLocked(const std::atomic_uint32_t & flag)
        {
            return owners(flag.load(std::memory_order_acquire)) != 0;
        }

        inline bool SpinLockUtil::isWriteLocked(const std::atomic_uint32_t & flag)
        {
            return owners(flag.load(std::memory_order_acquire)) == -1;
        }

        inline uint16_t SpinLockUtil::numReaders(const std::atomic_uint32_t & flag)
        {
            uint32_t num = flag.load(std::memory_order_acquire);
            return owners(num) == -1 ? 0 : owners(num);
        }

        inline uint16_t SpinLockUtil::numPendingWriters(const std::atomic_uint32_t & flag)
        {
            return upgrades(flag.load(std::memory_order_acquire));
        }

        inline uint32_t SpinLockUtil::set(int16_t upgrades, int16_t owners)
        {
            // return ((uint32_t)upgrades << 16) | (owners & mask);
            return static_cast<uint32_t>(upgrades << 16) | (owners & mask);

        }

        inline int16_t SpinLockUtil::upgrades(uint32_t n)
        {
            return (n >> 16) & mask;
        }

        inline int16_t SpinLockUtil::owners(uint32_t n)
        {
            return n & mask;
        }

        inline uint32_t SpinLockUtil::add(uint32_t n, int16_t upgrade, int16_t owner)
        {
