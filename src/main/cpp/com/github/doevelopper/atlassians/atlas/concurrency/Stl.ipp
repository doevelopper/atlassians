...
*/
#ifndef COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_STL_IMPL_H
#define COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_STL_IMPL_H
...
{
        template <std::size_t N, typename T, std::size_t S>
        struct TupleElement
        {
        ...
        template <typename RET, typename FUNC, typename... ARGS, std::size_t... I, typename... T>
        RET apply_impl(FUNC && func, std::tuple<ARGS...> && tuple, std::index_sequence<I...>, T &&... t)
        {
            return std::forward<FUNC>(func)(
                std::forward<T>(t)..., std::forward<ARGS>(std::get<I>(std::move(tuple)))...);
        }

        template <typename RET, typename FUNC, typename... ARGS, typename... T>
        RET apply(FUNC && func, std::tuple<ARGS...> && tuple, T &&... t)
        {
            return apply_impl<RET>(
                std::forward<FUNC>(func), std::move(tuple), std::index_sequence_for<ARGS...> {}, std::forward<T>(t)...);
        }
}

#endif // COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_STL_IMPL_H
