...
*/
#ifndef COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_FUTURE_STATE_H
#define COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_FUTURE_STATE_H
...
namespace Bloomberg::quantum
{
        ...
        constexpr FutureState operator|(FutureState lhs, FutureState rhs)
        {
            // return FutureState((std::uint8_t)lhs | (std::uint8_t)rhs);
            return FutureState(static_cast<std::uint8_t>(lhs) | static_cast<std::uint8_t>(rhs));
        }
        ...
        inline void ThrowFutureException(FutureState state)
        {
            switch (state)
            {
            case FutureState::PromiseNotSatisfied:
                throw PromiseNotSatisfiedException();
            case FutureState::PromiseAlreadySatisfied:
                throw PromiseAlreadySatisfiedException();
            case FutureState::BrokenPromise:
                throw BrokenPromiseException();
            case FutureState::FutureAlreadyRetrieved:
                throw FutureAlreadyRetrievedException();
            case FutureState::NoState:
                throw NoStateException();
            case FutureState::BufferingData:
                throw BufferingDataException();
            case FutureState::BufferClosed:
                throw BufferClosedException();
            default:
                throw std::logic_error("Invalid future state");
            }
        }
}

#endif // COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_FUTURE_STATE_H
