...
*/
#ifndef COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_DISPATCHER_H
#define COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_DISPATCHER_H
...
namespace Bloomberg::quantum
{
        ...
        public:
            ...
            /*!
             * @brief Constructor.
             * @param[in] config The configuration for the Quantum dispatcher.
             */
             ...
             *      has the smallest number of concurrent coroutines executing at the time of the post.
             ...
             *      method, an std::function, a functor generated via std::bind or a lambda. The signature of the
             ...
             * @note This function is non-blocking and returns immediately. The returned thread context cannot be used
             *      to chain further coroutines.
             ...
             *      IQueue::QueueId::Any  as a value, which is equivalent to running the simpler version of post() above. Valid
             *      range is [0, numCoroutineThreads) or IQueue::QueueId::Any.
             * @param[in] isHighPriority If set to true, the coroutine will be scheduled to run immediately after the
             *      currently executing coroutine on 'queueId' has completed or has yielded.
             * @param[in] func Callable object.
             * @param[in] args Variable list of arguments passed to the callable object.
             * @return A pointer to a thread context object.
             * @note This function is non-blocking and returns immediately. The returned thread context cannot be used
             ...
             *      other coroutines which will run sequentially.
             * @warning User functions should *never* catch all exceptions with (...) as it may block coroutine stack
             *      unwind exceptions from propagating which are not derived from std::exception (see link at the top).
             */
            template <class RET = Deprecated, class FUNC, class... ARGS>
            ...
            /*!
             * @brief Post the first coroutine in a continuation chain to run asynchronously on a specific queue
             *  (thread).
             * @tparam RET Type of future returned by this coroutine.
             * @tparam FUNC Callable object type which will be wrapped in a coroutine. Can be a standalone function, a
             *      method, an std::function, a functor generated via std::bind or a lambda. The signature of the
             ...
             * @note This function is non-blocking and returns immediately. The passed function will not be wrapped in
             *      a coroutine.
             */
            template <class RET = Deprecated, class FUNC, class... ARGS>
            ...
            /*!
             * @brief Post a blocking IO (or long running) task to run asynchronously on a specific thread in the IO
             *      thread pool.
             * @tparam RET Type of future returned by this task.
             * @tparam FUNC Callable object type. Can be a standalone function, a method, an std::function,
             *      a functor generated via std::bind or a lambda. The signature of the callable
             *      object must strictly be 'int f(ThreadPromise<RET>::Ptr, ...)'.
             * @tparam ARGS Argument types passed to FUNC.
             * @param[in] queueId Id of the queue where this task should run. Note that the user can specify
             *      IQueue::QueueId::Any as a value, which is equivalent to running the simpler version of postAsyncIo()
             *      above. Valid range is [0, numCoroutineThreads) or IQueue::QueueId::Any.
             ...
             */
            template <class RET = Deprecated, class INPUT_IT, class FUNC, class = Traits::IsInputIterator<INPUT_IT>>
             ...
             */
            template <class RET = Deprecated, class INPUT_IT, class FUNC>
             ...
             * @param[in] first The start iterator to a list of items to be processed in the range [first,last).
             * @param[in] last The end iterator to a list of items (not inclusive).
             * @param[in] mapper The mapper function.
            ...
            template <
                class KEY = Deprecated, class MAPPED_TYPE = Deprecated, class REDUCED_TYPE = Deprecated,
                class MAPPER_FUNC, class REDUCER_FUNC, class INPUT_IT, class = Traits::IsInputIterator<INPUT_IT>>
            auto mapReduce(INPUT_IT first, size_t num, MAPPER_FUNC mapper, REDUCER_FUNC reducer)
                -> ThreadContextPtr<std::map<decltype(mappedKeyOf(mapper)), decltype(reducedTypeOf(reducer))>>;
            ...
            template <
                class KEY = Deprecated, class MAPPED_TYPE = Deprecated, class REDUCED_TYPE = Deprecated,
                class MAPPER_FUNC, class REDUCER_FUNC, class INPUT_IT, class = Traits::IsInputIterator<INPUT_IT>>
            auto mapReduceBatch(INPUT_IT first, size_t num, MAPPER_FUNC mapper, REDUCER_FUNC reducer)
                -> ThreadContextPtr<std::map<decltype(mappedKeyOf(mapper)), decltype(reducedTypeOf(reducer))>>;

            /*!
             * @brief Signal all threads to immediately terminate and exit. All other pending coroutines and IO tasks
             *      will not complete. Call this function for a fast shutdown of the dispatcher.
             ...
             * @param[in] queueId The queue number to query. Valid range is [0, numCoroutineThreads) for
             *      IQueue::QueueType::Coro, [0, numIoThreads) for IQueue::QueueType::IO and IQueue::QueueId::All for either.
             * @return The total number of queued tasks including the currently executing one.
             * @note IQueue::QueueId::Same is an invalid queue id. IQueue::QueueId::Any is only valid for IO queue
             *      type. When type IQueue::QueueType::All is specified, the queueId is not used and must be left at default
             *      value.
             */
            size_t size(IQueue::QueueType type = IQueue::QueueType::All, int queueId = static_cast<int>(IQueue::QueueId::All)) const;
             ...
             *      value.
             */
            bool empty(IQueue::QueueType type = IQueue::QueueType::All, int queueId = static_cast<int>(IQueue::QueueId::All)) const;

            /*!
             * @brief Drains all queues on this dispatcher object.
             * @param[in] timeout Maximum time for this function to wait. Set to -1 to wait indefinitely until all
             *      queues drain.
             * @param[in] isFinal If set to true, the dispatcher will not allow any more processing after the drain
             *      completes.
             ...
             */
            bool drain(std::chrono::milliseconds timeout = std::chrono::milliseconds(-1), bool isFinal = false);

            /*!
             * @brief Returns the number of underlying coroutine threads as specified in the constructor. If -1 was
             *      passed than this number essentially indicates the number of cores.
            ...
            QueueStatistics stats(
                IQueue::QueueType type = IQueue::QueueType::All, int queueId = static_cast<int>(IQueue::QueueId::All));
        ...
        private:
            ...
            template <class RET, class FUNC, class... ARGS>
            ThreadFuturePtr<RET> postAsyncIoImpl(int queueId, bool isHighPriority, FUNC && func, ARGS &&... args);

            // Members
            DispatcherCore m_dispatcher;
            std::atomic_bool m_drain;
            std::atomic_bool m_terminated;
        };
        inline log4cxx::LoggerPtr Dispatcher::logger =
            log4cxx::Logger::getLogger ( std::string ( "com.github.doevelopper.premisses.atlas.concurrency.Dispatcher" ) );
        using TaskDispatcher = Dispatcher; // alias
}

#include <com/github/doevelopper/atlassians/atlas/concurrency/Dispatcher.ipp>

#endif // COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_DISPATCHER_H
