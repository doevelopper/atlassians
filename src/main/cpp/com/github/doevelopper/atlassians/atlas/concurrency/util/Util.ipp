...
//                UNUSED(ex);
            ...
            yield.get() = static_cast<int>(ITask::RetCode::Exception);
            return static_cast<int>(ITask::RetCode::Exception);
        }

        template <typename RET, typename CAPTURE>
        int bindCoro2(Traits::Yield & yield, std::shared_ptr<CoroContext<RET>> ctx, CAPTURE && capture)
        {
            try
            {
                ctx->setYieldHandle(yield); // set coroutine yield
...
//                UNUSED(ex);
        ...
        template <typename RET, typename CAPTURE>
        int bindIo2(std::shared_ptr<Promise<RET>> promise, CAPTURE && capture)
        {
            try
            {
...
//                UNUSED(ex);
        ...
        Util::bindCaller(std::shared_ptr<Context<RET>> context, FUNC && func, ARGS &&... args)
        {
            ...
            auto capture = makeCapture<int>(
                std::forward<FUNC>(func), std::shared_ptr<Context<RET>>(context), std::forward<ARGS>(args)...);
            return makeCapture<int>(
                bindCoro<RET, decltype(capture)>, std::shared_ptr<Context<RET>>(context), std::move(capture));
        }
        ...
        {
            LOG4CXX_TRACE(logger, __LOG4CXX_FUNC__);
            auto capture = makeCapture<RET>(
                std::forward<FUNC>(func), Util::makeVoidContext<RET>(context), std::forward<ARGS>(args)...);
            return makeCapture<int>(
                bindCoro2<RET, decltype(capture)>, std::shared_ptr<Context<RET>>(context), std::move(capture));
        }

        template <class RET, class FUNC, class... ARGS>
        Function<int()> Util::bindIoCaller(std::shared_ptr<Promise<RET>> promise, FUNC && func, ARGS &&... args)
        {
            LOG4CXX_TRACE(logger, __LOG4CXX_FUNC__);
            auto capture = makeCapture<int>(
                std::forward<FUNC>(func), std::shared_ptr<Promise<RET>>(promise), std::forward<ARGS>(args)...);
            return makeCapture<int>(
                bindIo<RET, decltype(capture)>, std::shared_ptr<Promise<RET>>(promise), std::move(capture));
        }

        template <class RET, class FUNC, class... ARGS>
        Function<int()> Util::bindIoCaller2(std::shared_ptr<Promise<RET>> promise, FUNC && func, ARGS &&... args)
        {
            LOG4CXX_TRACE(logger, __LOG4CXX_FUNC__);
            auto capture = makeCapture<RET>(std::forward<FUNC>(func), std::forward<ARGS>(args)...);
            return makeCapture<int>(
                bindIo2<RET, decltype(capture)>, std::shared_ptr<Promise<RET>>(promise), std::move(capture));
        }

        template <class RET, class INPUT_IT, class FUNC>
        std::vector<RET> Util::forEachCoro(VoidContextPtr ctx, INPUT_IT inputIt, size_t num, FUNC && func)
        {
            LOG4CXX_TRACE(logger, __LOG4CXX_FUNC__);
        ...
        template <class RET, class INPUT_IT, class FUNC>
        std::vector<std::vector<RET>> Util::forEachBatchCoro(
        ...
        std::map<KEY, REDUCED_TYPE> Util::mapReduceCoro(
            ...
            const Functions::ReduceFunc<KEY, MAPPED_TYPE, REDUCED_TYPE> & reducer)
        {
            ...
            ReducedResults reducedResults =
                ctx->forEach(indexerOutput.begin(), indexerOutput.size(), reducer)->get(ctx);
        ...
        template <class KEY, class MAPPED_TYPE, class REDUCED_TYPE, class INPUT_IT>
        std::map<KEY, REDUCED_TYPE> Util::mapReduceBatchCoro(
            VoidContextPtr ctx, INPUT_IT inputIt, size_t num,
            const Functions::MapFunc<KEY, MAPPED_TYPE, INPUT_IT> & mapper,
            const Functions::ReduceFunc<KEY, MAPPED_TYPE, REDUCED_TYPE> & reducer)
        {
            ...
            // Reduce stage
            ReducedResults reducedResults =
                ctx->forEachBatch(indexerOutput.begin(), indexerOutput.size(), reducer)->get(ctx);
