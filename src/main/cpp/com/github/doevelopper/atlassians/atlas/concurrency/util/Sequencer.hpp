...
*/
#ifndef COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_SEQUENCER_H
#define COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_SEQUENCER_H
...
namespace Bloomberg::quantum
{
        ...
        *       noticable when quantum::Dispatcher has a lot of coroutine threads configured and long chains of dependent
        *       tasks are enqueued to quantum::Sequencer. If this becomes an issue, it's suggested to use
        *       quantum::experimental::Sequencer instead. quantum::experimenetal::Sequencer manages task ordering ourside of
        *       quantum::Dispatcher.
        ...
        *       Sequencer::enqueue/enqueueAll throws an exception. The opaque parameter can be used to distinguish one task
        *       from another.
        */
        ...
        public:
             ...
             *      coroutine associated with the same 'sequenceKey' completes. If there are none, it will run immediately.
             ...
             *      IQueue::QueueId::Any is intended to be used as queueId here, then it is suggested that the
             *      configured Any-coroutine-queue-range (@see Configuration::setCoroQueueIdRangeForAny) does not
             *      contain the control queue id.
             *
             * @warning The VoidContextPtr can be used to yield() or to post additional coroutines or IO tasks.
             *      However it should *not* be set and this will result in undefined behavior.
             */
             ...
             *       the simpler version of post() above. Valid range is [0, numCoroutineThreads) or IQueue::QueueId::Any.
             *
             * @param[in] isHighPriority If set to true, the sequencer coroutine will be scheduled right
             *      after the currently executing coroutine on 'queueId'.
             *
             ...
             * @details This method will post the coroutine on any thread available. The posted task is assumed to be
             *      associated with the entire universe of sequenceKeys already running or pending, which means that it will
             *      wait until all tasks complete. This task can be considered as having a 'universal' key.
             *
             * @tparam FUNC Callable object type which will be wrapped in a coroutine with signature
             *      'int(VoidContextPtr, Args...)'
             *
             ...
             * @return the statistics objects
             * @note The difference with the previous two statistics methods is that it aggregates stats on a per-task
             *      basis, not on per-key basis.
             */
        ...
        private:
            ...
            template <class FUNC, class... ARGS>
            static int waitForTwoDependents(
            ...
            template <class FUNC, class... ARGS>
            static int
            callPosted(VoidContextPtr ctx, void * opaque, const Sequencer & sequencer, FUNC && func, ARGS &&... args);

            static bool canTrimContext(const ICoroContextBasePtr & ctx, const ICoroContextBasePtr & ctxToValidate);
            static bool isPendingContext(const ICoroContextBasePtr & ctx, const ICoroContextBasePtr & ctxToValidate);

            Dispatcher & m_dispatcher;
            std::atomic_bool m_drain;
            int m_controllerQueueId;
            SequenceKeyData m_universalContext;
        ...
        inline log4cxx::LoggerPtr Sequencer<SequenceKey, Hash, KeyEqual, Allocator>::Sequencer::logger =
            log4cxx::Logger::getLogger ( std::string ( "com.github.doevelopper.premisses.atlas.concurrency.util.Sequencer" ) );
}

#include <com/github/doevelopper/atlassians/atlas/concurrency/util/Sequencer.ipp>

#endif // COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_SEQUENCER_H
