...
*/
#ifndef COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_CONDITION_VARIABLE_H
#define COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_CONDITION_VARIABLE_H
...
namespace Bloomberg::quantum
{
        ...
        public:
            ...
            /*!
             *  @brief Destructor.
             *  @warning Deleting this object while there are still waiting threads results in undefined behavior.
             *  Ensure that
             *           all threads have been notified before doing so. Once the destructor is called, no other wait
             *           attempts should be made on this object.
             */
            ~ConditionVariable();
            ...
            /*!
             *  @brief Block the current thread until the condition is signalled via notifyOne() or notifyAll().
             *  @details When this function returns, the mutex is guaranteed to be locked. This function calls wait() in
             *      a loop until the predicate returns true. This ensures that the condition has not changed after
             *      notifyOne() or notifyAll() have been called. The internal logic is equivalent to:
             *  @code
             *       while(!predicate())
             *      {
             *          wait(mutex);
            ...
            /*!
             *  @brief Yield the current coroutine until the condition is signalled via notifyOne() or notifyAll(). When
             *      this function returns, the mutex is guaranteed to be locked.
             *  @details When this function returns, the mutex is guaranteed to be locked. This function calls wait() in
             *      a loop until the predicate returns true. This ensures that the condition has not changed after
             *      notifyOne() or notifyAll() have been called. The internal logic is equivalent to:
             *  @code
            ...
            /*!
             *  @brief Block the current thread until the condition is signalled via notifyOne() or notifyAll() or until
             ...
             *           wait() in a loop until the predicate returns true or 'time' expires. This ensures that the
             *           condition has not changed after notifyOne() or notifyAll() have been called. The internal logic
             *           is equivalent to:
             *  @code
             *      while(!predicate())
             *      {
             ...
             *      Using -1 is equivalent to calling wait(). Other negative values will throw.
             *  @param[in] predicate Function or functor to be tested as exit condition of the endless while loop.
             *  @return True if the mutex was acquired before 'time' expired, otherwise the predicate result after
             *      timeout.
             *  @note This function should be called from a regular thread not from a coroutine.
             */
             ...
             *           has not changed after notifyOne() or notifyAll() have been called. The internal logic is
             *           equivalent to:
             *  @code
             *      while(!predicate())
             *      {
        ...
        private:
            ...
            void waitImpl(ICoroSync::Ptr sync, Mutex & mutex);

            template <class PREDICATE = bool()>
            void waitImpl(ICoroSync::Ptr sync, Mutex & mutex, PREDICATE predicate);

            template <class REP, class PERIOD>
            bool waitForImpl(ICoroSync::Ptr sync, Mutex & mutex, const std::chrono::duration<REP, PERIOD> & time);
            ...
            // MEMBERS
            Mutex m_thisLock; // sync access to this object
            std::list<std::atomic_int *> m_waiters;
            std::atomic_bool m_destroyed;
            LOG4CXX_DECLARE_STATIC_LOGGER
        };
        inline log4cxx::LoggerPtr ConditionVariable::logger =
        log4cxx::Logger::getLogger ( std::string ( "com.github.doevelopper.premisses.atlas.concurrency.ConditionVariable" ) );
}

#include <com/github/doevelopper/atlassians/atlas/concurrency/ConditionVariable.ipp>

#endif // COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_CONDITION_VARIABLE_H
