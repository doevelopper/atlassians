...
*/
#ifndef COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_SEQUENCER_EXPERIMENTAL_H
#define COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_SEQUENCER_EXPERIMENTAL_H

#if __cplusplus >= 201402L
    #define COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_SEQUENCER_SUPPORT
#endif // __cplusplus >= 201402L

#ifdef COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_SEQUENCER_SUPPORT
...
namespace Bloomberg::quantum::experimental
{
             ...
             * @note The interfaces of quantum::experimental::Sequencer and quantum::Sequencer are the same. The major
             *      difference between their functionalities is that quantum::experimental::Sequencer does not rely on
             *      quantum::Dispatcher to order tasks based on their interdependence. In contrast,
             *      quantum::experimental::Sequencer manages task ordering itself by construcing a DAG of pending tasks. In
             *      quantum::experimental::Sequencer, a task is pushed into quantum::Dispatcher only when it's ready to be
             *      executed (i.e. when it has no pending dependents). This typically results in executing scheduled tasks
             *      faster (w.r.t. quantum::Dispatcher) and wasting fewer CPU cycles in quantum::Dispatcher.
             *
             * @note Due to the fact that tasks enqueued to Sequencer do not get sent to quantum::Dispatcher right
             *      away, no enqueue/enqueueAll method of Sequencer returns an instance of ThreadContextPtr. An important
             *      goal of ThreadContextPtr returned by quantum::Dispather::post(...) calls is exception marshalling i.e.
             *      notifing the caller of an exception thrown in a coroutine. Sequencer uses the exception-callback
             *      mechanism instead. A user can specify an exception callback (see
             *      SequencerConfiguration<...>::getExceptionCallback) that will be called whenever a task posted to
             *      Sequencer::enqueue/enqueueAll throws an exception. The opaque parameter can be used to distinguish one
             *      task from another.
             */
            ...
            public:
                ...
                * @details This method will post the coroutine on any thread available and will run when the previous
                *       coroutine associated with the same 'sequenceKey' completes. If there are none, it will run
                *       immediately. (@see Dispatcher::post for more details).
                * @tparam FUNC Callable object type which will be wrapped in a coroutine with signature
                *       'int(VoidContextPtr, Args...)'
                *
                ...
                */
                ...
                void enqueue(const SequenceKey & sequenceKey, FUNC && func, ARGS &&... args);
                ...
                * @brief Enqueue a coroutine to run asynchronously.
                * @details This method will post the coroutine on any thread available and will run when the previous
                *       coroutine(s) associated with all the 'sequenceKeys' complete. If there are none, then it will run
                *       immediately. (@see Dispatcher::post for more details).
                *
                * @tparam FUNC Callable object type which will be wrapped in a coroutine with signature
                *       'int(VoidContextPtr, Args...)'
                ...
                */
                template <class FUNC, class... ARGS>
                void enqueue(const std::vector<SequenceKey> & sequenceKeys, FUNC && func, ARGS &&... args);
                ...
                * @details This method will post the coroutine on any thread available. The posted task is assumed to
                *       be associated with the entire universe of sequenceKeys already running or pending, which means that it
                *       will wait until all tasks complete. This task can be considered as having a 'universal' key.
                * @tparam FUNC Callable object type which will be wrapped in a coroutine with signature
                *       'int(VoidContextPtr, Args...)'
                * @tparam ARGS Argument types passed to FUNC (@see Dispatcher::post for more details).
                * @param[in] func Callable object.
                ...
                */
                template <class FUNC, class... ARGS>
                void enqueueAll(FUNC && func, ARGS &&... args);
                ...
                */
                template <class FUNC, class... ARGS>
                void enqueueAll(void * opaque, int queueId, bool isHighPriority, FUNC && func, ARGS &&... args);
                ...
                * @note This function blocks until the statistics computation job posted to the dispatcher is
                *       finished.
                */
                ...
                * @return the statistics objects
                * @note The difference with the previous two statistics methods is that it aggregates stats on a
                *       per-task basis,not on per-key basis.
                */
                ...
                *       sequences drain.
                * @param[in] isFinal If set to true, the sequencer will not allow any more processing after the drain
                *       completes.
                ...
                *       Since this function posts a task which will wait on all others,
                *       getStatistics().getPostedTaskCount() will contain one extra count.
                */
                bool drain(std::chrono::milliseconds timeout = std::chrono::milliseconds(-1), bool isFinal = false);

            private:

                using PendingTaskQueueMap =
                    std::unordered_map<SequenceKey, SequencerKeyData<SequenceKey>, Hash, KeyEqual, Allocator>;
                ...
                */
                bool addPendingTask(const SequenceKey & key, const std::shared_ptr<SequencerTask<SequenceKey>> & task);
                ...
                */
                void scheduleTask(const std::shared_ptr<SequencerTask<SequenceKey>> & task);
                ...
                */
                ...
                template <class FUNC, class... ARGS>
                void enqueueAllImpl(void * opaque, int queueId, bool isHighPriority, FUNC && func, ARGS &&... args);

                Dispatcher & m_dispatcher;
                std::atomic_bool m_drain;
                SequencerKeyData<SequenceKey> m_universalTaskQueue;
                PendingTaskQueueMap m_pendingTaskQueueMap;
                ExceptionCallback m_exceptionCallback;
                quantum::Mutex m_mutex;
                std::shared_ptr<SequenceKeyStatisticsWriter> m_taskStats;
                LOG4CXX_DECLARE_STATIC_LOGGER
            };

            template <class SequenceKey, class Hash, class KeyEqual, class Allocator>
            inline log4cxx::LoggerPtr Sequencer<SequenceKey, Hash, KeyEqual, Allocator>::logger =
                log4cxx::Logger::getLogger ( std::string ( "com.github.doevelopper.premisses.atlas.concurrency.Sequencer" ) );
}

#include <com/github/doevelopper/atlassians/atlas/concurrency/util/SequencerExperimental.ipp>

#endif // COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_SEQUENCER_SUPPORT

#endif // COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_SEQUENCER_EXPERIMENTAL_H
