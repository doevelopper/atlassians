...
#ifndef COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_QUOTA_DEFERREDRECLAMATIONALLOCATOR_HPP
#define COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_QUOTA_DEFERREDRECLAMATIONALLOCATOR_HPP
...
namespace com::github::doevelopper::atlassians::atlas::quota
{
    ...
    //! `DeferredReclamationAllocator` is meant to wrap another, underlying,
    ...
    class DeferredReclamationAllocator
    {
    ...
    public:
        ...
        struct rebind
        {
            using other = DeferredReclamationAllocator<alloc_rebind_t<T>>;
        };

        DeferredReclamationAllocator() = delete;
        ...
        //!        This must be an integer greater than 0.
        template <typename Rep, typename Period>
        DeferredReclamationAllocator(std::chrono::duration<Rep, Period> timeout, std::size_t delay_buffer_size = 100)
            : DeferredReclamationAllocator {Allocator {}, timeout, delay_buffer_size}
        {
        }

        //! Converting copy-constructor from another `DeferredReclamationAllocator`.
        //!
        //! This allows constructing a `DeferredReclamationAllocator` from another
        ...
        DeferredReclamationAllocator(
            DeferredReclamationAllocator<OtherAllocator> const & other, force_copy_tag /* ignore this */ = {})
            : allocator_ {other.allocator_}
            , timeout_ {other.timeout_}
            , now_ {TimeoutClock::now()}
            , m_bufferAllocator {other.m_bufferAllocator}
        ...
        //! Copy constructs an allocator from another `DeferredReclamationAllocator`.
        //!
        //! For details, see notes on copy-constructibility in the class-wide
        //! documentation.
        DeferredReclamationAllocator(DeferredReclamationAllocator const & other)
            : DeferredReclamationAllocator {other, force_copy_tag {}}
        {
        }

        //! Move constructs a `DeferredReclamationAllocator` from another one.
        //!
        ...
        //!
        //! A moved-from `DeferredReclamationAllocator` may only be destroyed;
        //! calling any other method is undefined behavior.
        DeferredReclamationAllocator(DeferredReclamationAllocator && other)
            ...
            , m_currentBuffer {std::exchange(other.m_currentBuffer, nullptr)}
            , m_delayList {std::move(other.m_delayList)}
        {
        }

        DeferredReclamationAllocator & operator=(DeferredReclamationAllocator const &) = delete;
        DeferredReclamationAllocator & operator=(DeferredReclamationAllocator &&)      = delete;
        ...
        //! ```c++
        //! DeferredReclamationAllocator<...> allocator{...};
        //!
        ...
        void deallocate(pointer p, std::size_t n) noexcept(std::is_nothrow_destructible<value_type> {})
        {
            ...
            assert(
                !current_buffer_full()
                && "the buffer should never be full when entering `deallocate()`, "
            ...
            if (current_buffer_full())
            {
                ...
                now_ = m_currentBuffer->timestamp = TimeoutClock::now();
                m_delayList.push_back(*std::exchange(m_currentBuffer, nullptr)); // intrusive list; does not throw

                // 2. Try to reuse an existing buffer by purging the delay list.
                m_currentBuffer = purge_delay_list_and_reuse_existing_buffer();
                ...
                if (m_currentBuffer == nullptr)
                {
                    try
                    {
                        m_currentBuffer = buffer_new(); // strong exception guarantee
                    }
                    catch (std::bad_alloc const &)
                    {
                        ...
                        assert(
                            !m_delayList.empty()
                            && "we just pushed back the latest buffer to the delay "
                               "list, so there should be at least one element");
                        auto const & oldest = m_delayList.front();
                ...
                m_currentBufferSize = 0; // mark the current buffer as being empty
            }
        }
        ...
        ~DeferredReclamationAllocator() noexcept(std::is_nothrow_destructible<value_type> {})
        {
            ...
            {
                auto const ready_to_delete = m_currentBuffer->timestamp + timeout_;
                if (now <= ready_to_delete)
                {
                    std::this_thread::sleep_until(ready_to_delete);
                }
                reclaim_buffer_elements(m_currentBuffer->elements, m_currentBuffer->elements + m_currentBufferSize);
            }
            buffer_delete(m_currentBuffer);
        }

        //! Returns whether an allocator can be used to deallocate storage allocated
        //! by another allocator.
        //!
        //! Two `DeferredReclamationAllocator`s are equal if and only if their
        ...
        //! documentation.
        friend bool operator==(DeferredReclamationAllocator const & a, DeferredReclamationAllocator const & b)
        {
        ...
        {
            static_assert(
                std::is_same<Flavor, detail::opportunistic_t> {} || std::is_same<Flavor, detail::exhaustive_t> {},
                "'DeferredReclamationAllocator::purge' has two flavor: opportunistic and exhaustive. pick one.");
            assert(!was_moved_from());

            auto const reclaim_full_buffer = [this](alloc_pointer_t<DelayBuffer> buffer) {
                reclaim_buffer_elements(buffer->elements, buffer->elements + m_bufferCapacity);
                buffer_delete(buffer);
            };

            now_ = TimeoutClock::now();

            while (!m_delayList.empty())
            {
                auto & oldest = m_delayList.front();
                ...
                {
                    m_delayList.pop_front_and_dispose(reclaim_full_buffer);
                }
    ...
    private:
        ...
        template <typename>
        friend class DeferredReclamationAllocator;
        ...
        static_assert(
            noexcept(TimeoutClock::now()), "We make the assumption that our clock does not throw in various places "
        ...
        struct DelayBuffer : boost::intrusive::slist_base_hook<
        ...
        void current_buffer_push_back(DelayBufferElement elem) noexcept
        {
            ...
            m_currentBuffer->elements[m_currentBufferSize] = elem;
            ++m_currentBufferSize;
        }
        ...
        {
            std::size_t const buffer_element_bytes = m_bufferCapacity * sizeof(DelayBufferElement);
            alloc_pointer_t<char> bytes = m_bufferAllocator.allocate(sizeof(DelayBuffer) + buffer_element_bytes);
        ...
        void buffer_delete(alloc_pointer_t<DelayBuffer> buffer)
        {
            ...
            m_bufferAllocator.deallocate(bytes, m_bufferCapacity);
        }
        ...
        alloc_pointer_t<DelayBuffer> purge_delay_list_and_reuse_existing_buffer()
        {
            ...
            while (!m_delayList.empty())
            {
                ...
                reclaim_buffer_elements(oldest.elements, oldest.elements + m_bufferCapacity);
                m_delayList.pop_front(); // does not throw or invalidate references
        ...
        std::size_t const m_bufferCapacity;
        std::size_t m_currentBufferSize;
        alloc_pointer_t<DelayBuffer> m_currentBuffer; // `nullptr` iff `*this` has been moved-from
        DelayList m_delayList;
