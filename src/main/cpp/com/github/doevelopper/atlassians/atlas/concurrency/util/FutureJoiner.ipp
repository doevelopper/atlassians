...
namespace Bloomberg::quantum
{
        ...
        ThreadFuturePtr<std::vector<T>> FutureJoiner<T>::join(
            ThreadContextTag, DISPATCHER & dispatcher, std::vector<typename FUTURE<T>::Ptr> && futures)
        {
            LOG4CXX_TRACE(logger, __LOG4CXX_FUNC__);
#if (__cplusplus == 201103L)
            std::shared_ptr<std::vector<typename FUTURE<T>::Ptr>> containerPtr(
                new std::vector<typename FUTURE<T>::Ptr>(std::move(futures)));
