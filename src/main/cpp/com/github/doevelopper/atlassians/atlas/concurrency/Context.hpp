...
*/
#ifndef COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_CONTEXT_H
#define COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_CONTEXT_H
...
namespace Bloomberg::quantum
{
        ...
        public:
            ...
            typename Context<OTHER_RET>::Ptr post(FUNC && func, ARGS &&... args);

            template <class OTHER_RET, class FUNC, class... ARGS>
            typename Context<OTHER_RET>::Ptr post2(FUNC && func, ARGS &&... args);

            template <class OTHER_RET, class FUNC, class... ARGS>
            typename Context<OTHER_RET>::Ptr post(int queueId, bool isHighPriority, FUNC && func, ARGS &&... args);

            template <class OTHER_RET, class FUNC, class... ARGS>
            typename Context<OTHER_RET>::Ptr post2(int queueId, bool isHighPriority, FUNC && func, ARGS &&... args);

            template <class OTHER_RET, class FUNC, class... ARGS>
            typename Context<OTHER_RET>::Ptr postFirst(FUNC && func, ARGS &&... args);

            template <class OTHER_RET, class FUNC, class... ARGS>
            typename Context<OTHER_RET>::Ptr postFirst2(FUNC && func, ARGS &&... args);

            template <class OTHER_RET, class FUNC, class... ARGS>
            typename Context<OTHER_RET>::Ptr postFirst(int queueId, bool isHighPriority, FUNC && func, ARGS &&... args);
            ...
            template <class OTHER_RET, class FUNC, class... ARGS>
            typename Context<OTHER_RET>::Ptr then(FUNC && func, ARGS &&... args);

            template <class OTHER_RET, class FUNC, class... ARGS>
            typename Context<OTHER_RET>::Ptr then2(FUNC && func, ARGS &&... args);

            template <class OTHER_RET, class FUNC, class... ARGS>
            typename Context<OTHER_RET>::Ptr onError(FUNC && func, ARGS &&... args);

            template <class OTHER_RET, class FUNC, class... ARGS>
            typename Context<OTHER_RET>::Ptr onError2(FUNC && func, ARGS &&... args);

            template <class OTHER_RET, class FUNC, class... ARGS>
            typename Context<OTHER_RET>::Ptr finally(FUNC && func, ARGS &&... args);

            template <class OTHER_RET, class FUNC, class... ARGS>
            typename Context<OTHER_RET>::Ptr finally2(FUNC && func, ARGS &&... args);
            ...
            template <class OTHER_RET, class FUNC, class... ARGS>
            CoroFuturePtr<OTHER_RET> postAsyncIo2(FUNC && func, ARGS &&... args);

            template <class OTHER_RET, class FUNC, class... ARGS>
            CoroFuturePtr<OTHER_RET> postAsyncIo(int queueId, bool isHighPriority, FUNC && func, ARGS &&... args);

            template <class OTHER_RET, class FUNC, class... ARGS>
            CoroFuturePtr<OTHER_RET> postAsyncIo2(int queueId, bool isHighPriority, FUNC && func, ARGS &&... args);

            //===================================
            //           FOR EACH
            //===================================
            template <class OTHER_RET, class INPUT_IT, class FUNC, class = Traits::IsInputIterator<INPUT_IT>>
            typename Context<std::vector<OTHER_RET>>::Ptr forEach(INPUT_IT first, INPUT_IT last, FUNC && func);

            template <class OTHER_RET, class INPUT_IT, class FUNC>
            typename Context<std::vector<OTHER_RET>>::Ptr forEach(INPUT_IT first, size_t num, FUNC && func);
            ...
            //===================================
            template <
                class KEY, class MAPPED_TYPE, class REDUCED_TYPE, class INPUT_IT,
                class = Traits::IsInputIterator<INPUT_IT>>
            typename Context<std::map<KEY, REDUCED_TYPE>>::Ptr mapReduce(
            ...
            template <
                class KEY, class MAPPED_TYPE, class REDUCED_TYPE, class INPUT_IT,
                class = Traits::IsInputIterator<INPUT_IT>>
            typename Context<std::map<KEY, REDUCED_TYPE>>::Ptr mapReduceBatch(
                INPUT_IT first, INPUT_IT last, Functions::MapFunc<KEY, MAPPED_TYPE, INPUT_IT> mapper,
                Functions::ReduceFunc<KEY, MAPPED_TYPE, REDUCED_TYPE> reducer);
        ...
        private:
            ...
            template <class OTHER_RET, class FUNC, class... ARGS>
            typename Context<OTHER_RET>::Ptr thenImpl(ITask::Type type, FUNC && func, ARGS &&... args);
            ...
            template <class OTHER_RET, class FUNC, class... ARGS>
            CoroFuturePtr<OTHER_RET> postAsyncIoImpl(int queueId, bool isHighPriority, FUNC && func, ARGS &&... args);
            ...
            // Members
            ITask::Ptr m_task;
            std::vector<IPromiseBase::Ptr> m_promises;
            DispatcherCore * m_dispatcher;
            std::atomic_bool m_terminated;
            std::atomic_int m_signal;
            Traits::Yield * m_yield;
            std::chrono::microseconds m_sleepDuration;
            std::chrono::steady_clock::time_point m_sleepTimestamp;

            LOG4CXX_DECLARE_STATIC_LOGGER
        };
        template <class RET>
        inline log4cxx::LoggerPtr Context<RET>::logger =
            log4cxx::Logger::getLogger ( std::string ( "com.github.doevelopper.premisses.atlas.concurrency.Context" ) );
        template <class RET>
        using ContextPtr = typename Context<RET>::Ptr;
}

#include <com/github/doevelopper/atlassians/atlas/concurrency/Context.ipp>
#include <com/github/doevelopper/atlassians/atlas/concurrency/interface/IContext.hpp>

#endif // COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_CONTEXT_H
