...
*/
#ifndef COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_ICORO_CONTEXT_H
#define COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_ICORO_CONTEXT_H
...
namespace Bloomberg::quantum
{
        ...
        */
        ...
        struct ICoroContext : ICoroContextBase
        {
            ...
            /// @note Blocks until the future is ready or until an exception is thrown. Once this function returns, the
            /// future
            ...
            /// @note Blocks until the future is ready or until an exception is thrown. Contrary to get(), this function
            /// does
            ...
            /// @note Contrary to get() or getRef() this function never blocks since the previous coroutine is
            /// guaranteed
            ///       to have completed. Thus the future, if any, is already set. Once this function returns, the future
            ///       becomes invalidated.
            template <class OTHER_RET>
            NonBufferRetType<OTHER_RET> getPrev();

            /// @brief Get a reference to future value associated with the previous coroutine context in the
            /// continuation chain.
            /// @tparam OTHER_RET The type of the future value of the previous context.
            ...
            ///          getAt(total_continuations-1) on the last context in the chain (i.e. the context which is
            ///          returned via end()). Position 0 represents the first future in the chain.
            ...
            /// @note Blocks until the future is ready or until an exception is thrown. Contrary to getAt() this
            /// function will
            ...
            /// @tparam V Type of the promised value. This should be implicitly deduced by the compiler and should
            /// always == RET.
            ...
            /// @tparam V The type of value contained in Buffer.
            /// @param[in] sync Pointer to the coroutine synchronization object.
            /// @param[out] isBufferClosed Indicates if this buffer is closed and no more Pull operations are allowed on
            /// it.
            ...
            /// @tparam BUF Represents a class of type Buffer.
            /// @note Once closed no more Pushes can be made into the buffer. The corresponding future can still Pull
            /// values until
            ///       the buffer is empty.
            /// @return 0 on success.
            ...
            //-----------------------------------------------------------------------------------------
            /// @attention
            /// Continuation methods are typically chained in the following manner and must follow the relative
            /// placement below. postFirst() and end() are the only mandatory methods. onError() and finally() can be
            /// called at most once, whereas then() may be called zero or more times.
            ///
            /// @code
            ///    ICoroContext<RET>::Ptr ctx =
            ///    ICoroContext::postFirst()->then()->...->then()->onError()->finally()->end();
            /// @endcode
            ///
            ...
            /// @details This method will post the coroutine on any thread available. Typically it will pick one which
            /// has the
            ...
            /// @tparam FUNC Callable object type which will be wrapped in a coroutine. Can be a standalone function, a
            /// method,
            ///              an std::function, a functor generated via std::bind or a lambda. The signature of the
            ///              callable object must strictly be 'int f(CoroContext<RET>::Ptr, ...)'.
            ...
            /// @note This function is non-blocking and returns immediately. The returned context cannot be used to
            /// chain
            ///       further coroutines.
            ...
            ///              callable object must strictly be 'int f(CoroContext<RET>::Ptr, ...)'.
            /// @tparam ARGS Argument types passed to FUNC.
            /// @param[in] queueId Id of the queue where this coroutine should run. Note that the user can specify
            /// IQueue::QueueId::Any
            ///                    as a value, which is equivalent to running the simpler version of post() above. Valid
            ///                    range is [0, numCoroutineThreads), IQueue::QueueId::Any or IQueue::QueueId::Same.
            ///                    When using IQueue::QueueId::Same as input, the current queueId of this execution
            ///                    context will be preserved for the posted coroutine thus enabling to write lock-free
            ///                    code.
            /// @param[in] isHighPriority If set to true, the coroutine will be scheduled to run immediately after the
            /// currently
            ...
            /// @details This function is the head of a coroutine continuation chain and must be called only once in the
            /// chain.
            /// @tparam OTHER_RET Type of future returned by this coroutine.
            /// @tparam FUNC Callable object type which will be wrapped in a coroutine. Can be a standalone function, a
            /// method,
            ...
            ///       further coroutines. Possible method calls following this are then(), onError(), finally() and
            ///       end().
            template <class OTHER_RET = Deprecated, class FUNC, class... ARGS>
            ...
            /// @brief Posts a coroutine to run asynchronously.
            /// @details This function is optional for the continuation chain and may be called 0 or more times. If
            /// called,
            ...
            /// @brief Posts a coroutine to run asynchronously. This is the error handler for a continuation chain and
            /// acts as
            ///        as a 'catch' clause.
            /// @details This function is optional for the continuation chain and may be called at most once. If called,
            ///          it must follow postFirst() or another then() method. This method will conditionally run
            ///          if-and-only-if any previous coroutines in the continuation chain return an error or throw. When
            ///          a coroutine which is part of a continuation chain has an error, all subsequent then() methods
            ///          are skipped and if onError() is provided it will be called. If there are no errors, this method
            ///          is skipped.
            /// @tparam OTHER_RET Type of future returned by this coroutine.
            /// @tparam FUNC Callable object type which will be wrapped in a coroutine. Can be a standalone function, a
            /// method,
            ...
            /// @details This function is optional for the continuation chain and may be called at most once. If called,
            /// it must
            ///          immediately precede the end() method. This method will run regardless if any preceding
            ///          coroutines have an error or not. It can be used for cleanup purposes, closing handles,
            ...
            /// @param[in] args Variable list of arguments passed to the callable object.
            /// @return A pointer to a coroutine context object.
            /// @note This function is non-blocking and returns immediately. After this coroutine, the end() method must
            /// be called.
            template <class OTHER_RET = Deprecated, class FUNC, class... ARGS>
            ...
            /// @details This method effectively closes the continuation chain and posts the entire chain to be
            /// executed,
            ///          respecting the 'queueId' and priority specified at the beginning of the chain (see
            ///          postFirst()).
            /// @return Pointer to this context.
            /// @note This method does not take any functions as parameter as it is strictly used for scheduling
            /// purposes.
            Ptr end();
            ...
            /// @param[in] func Callable object.
            /// @param[in] args Variable list of arguments passed to the callable object.
            /// @return A pointer to a coroutine future object which may be used to retrieve the result of the IO
            /// operation.
            ...
            ///              'int f(ThreadPromise<RET>::Ptr, ...)'.
            /// @tparam ARGS Argument types passed to FUNC.
            /// @param[in] queueId Id of the queue where this function should run. Note that the user can specify
            /// IQueue::QueueId::Any
            ///                    as a value, which is equivalent to running the simpler version of postAsyncIo()
            ///                    above. Valid range is [0, numIoThreads) or IQueue::QueueId::Any.
            ///                    IQueue::QueueId::Same is disallowed.
            /// @param[in] isHighPriority If set to true, the function will be scheduled to run immediately after the
            /// currently
            ...
            template <
                class OTHER_RET = Deprecated, class INPUT_IT, class FUNC, class = Traits::IsInputIterator<INPUT_IT>>
            ...
            template <class OTHER_RET = Deprecated, class INPUT_IT, class FUNC>
            auto forEach(INPUT_IT first, std::size_t num, FUNC && func) ->
                typename ICoroContext<std::vector<decltype(coroResult(func))>>::Ptr;
            ...
            ///        is not a random access iterator.
            template <class OTHER_RET = Deprecated, class INPUT_IT, class FUNC>
            auto forEachBatch(INPUT_IT first, std::size_t num, FUNC && func) ->
                typename ICoroContext<std::vector<std::vector<decltype(coroResult(func))>>>::Ptr;
            ...
            ///          However it should *not* be set and this will result in undefined behavior.
            template <
                class KEY = Deprecated, class MAPPED_TYPE = Deprecated, class REDUCED_TYPE = Deprecated,
                class MAPPER_FUNC, class REDUCER_FUNC, class INPUT_IT, class = Traits::IsInputIterator<INPUT_IT>>
            auto mapReduce(INPUT_IT first, INPUT_IT last, MAPPER_FUNC mapper, REDUCER_FUNC reducer) ->
            ...
            template <
                class KEY = Deprecated, class MAPPED_TYPE = Deprecated, class REDUCED_TYPE = Deprecated,
                class MAPPER_FUNC, class REDUCER_FUNC, class INPUT_IT, class = Traits::IsInputIterator<INPUT_IT>>
            auto mapReduce(INPUT_IT first, std::size_t num, MAPPER_FUNC mapper, REDUCER_FUNC reducer) ->
                typename ICoroContext<std::map<decltype(mappedKeyOf(mapper)), decltype(reducedTypeOf(reducer))>>::Ptr;
            ...
            template <
                class KEY = Deprecated, class MAPPED_TYPE = Deprecated, class REDUCED_TYPE = Deprecated,
                class MAPPER_FUNC, class REDUCER_FUNC, class INPUT_IT, class = Traits::IsInputIterator<INPUT_IT>>
            auto mapReduceBatch(INPUT_IT first, INPUT_IT last, MAPPER_FUNC mapper, REDUCER_FUNC reducer) ->
                typename ICoroContext<std::map<decltype(mappedKeyOf(mapper)), decltype(reducedTypeOf(reducer))>>::Ptr;
            ...
            template <
                class KEY = Deprecated, class MAPPED_TYPE = Deprecated, class REDUCED_TYPE = Deprecated,
                class MAPPER_FUNC, class REDUCER_FUNC, class INPUT_IT, class = Traits::IsInputIterator<INPUT_IT>>
            auto mapReduceBatch(INPUT_IT first, std::size_t num, MAPPER_FUNC mapper, REDUCER_FUNC reducer) ->
                typename ICoroContext<std::map<decltype(mappedKeyOf(mapper)), decltype(reducedTypeOf(reducer))>>::Ptr;
        ...
        inline log4cxx::LoggerPtr ICoroContext<RET>::logger =
            log4cxx::Logger::getLogger ( std::string ( "com.github.doevelopper.premisses.atlas.concurrency.ICoroContext" ) );
        ...
        template <class RET>
        struct Traits::InnerType<std::shared_ptr<ICoroContext<RET>>>
        {
            using Type = RET;
        };
...
#endif // COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_ICORO_CONTEXT_H
