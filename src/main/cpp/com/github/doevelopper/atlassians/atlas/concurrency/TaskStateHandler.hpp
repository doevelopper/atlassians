...
*/
#ifndef COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_TASK_STATE_HANDLER_H
#define COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_TASK_STATE_HANDLER_H
...
namespace Bloomberg::quantum
{
        ...
        enum class TaskType : int
        {
            ...
            All       = Coroutine | IoTask ///< All task types
        };
         ...
         *      If the task isn't completed (the execution control is passed to another coroutine) it goes into
         *      'Suspended' state, otherwise upon the completion or termination due to an exception it switches to
         *      'Stopped' state. If the task dwells in 'Suspended' state and scheduled for execution it goes into
         *      'Resumed' state. The switch between 'Suspended' and 'Resumed' states may take several times until the
         *      task eventually switches over to 'Stopped' state.
         ...
         *      grind the whole execution to a halt. Only thread local variables or pre-allocated arrays that could
         *      use the queue id as index.
         */
        enum class TaskState : int
        {
            ...
            All         = Started | Suspended | Resumed | Stopped ///< All task states
        };
        ...
        private:
            ...
            TaskStateHandler m_handlerm; // task state handler
            TaskState m_handledStates {TaskState::None}; // task states that will be handled
            TaskType m_handledTypes {TaskType::None}; // task types that will be handled
            LOG4CXX_DECLARE_STATIC_LOGGER
        };
        inline log4cxx::LoggerPtr TaskStateConfiguration::logger =
        log4cxx::Logger::getLogger ( std::string ( "com.github.doevelopper.premisses.atlas.concurrency.TaskStateConfiguration" ) );
        /*!
...
#include <com/github/doevelopper/atlassians/atlas/concurrency/TaskStateHandler.ipp>

#endif // COM_GITHUB_DOEVELOPPER_PREMISSES_ATLAS_CONCURRENCY_TASK_STATE_HANDLER_H
