...
#include <com/github/doevelopper/atlassians/atlas/concurrency/SequencerTest.hpp>
...
log4cxx::LoggerPtr SequencerTest::logger =
    log4cxx::Logger::getLogger ( std::string ( "com.github.doevelopper.atlassians.atlas.concurrency.test.SequencerTest" ) );
...
TEST_P(SequencerTest, ExceptionHandler)
{
    ...
    std::generate(sequenceKeys.begin(), sequenceKeys.end(), [&generateTaskId]() { return generateTaskId++; });
    ...
    for (SequencerTestData::TaskId id = 0; id < taskCount; ++id)
    {
        ...
        if (id % exceptionFrequency == 0)
        {
            ...
            sequencer.enqueue(
                &sequenceKeys[id], (int)IQueue::QueueId::Any, false, sequenceKey,
                testData.makeTaskWithException(id, errorText));
...
TEST_P(SequencerTest, SequenceKeyStats)
{
    ...
    auto halfWayDoneJob = [](VoidContextPtr) -> int { return 0; };
...
TEST_P(SequencerTest, CustomHashFunction)
{
    ...
    using KeyEqual             = std::function<bool(
        SequencerTestData::SequenceKey sequenceKeyId0, SequencerTestData::SequenceKey sequenceKeyId1)>;
    const KeyEqual customEqual = [customHasher](
                                     SequencerTestData::SequenceKey sequenceKeyId0,
                                     SequencerTestData::SequenceKey sequenceKeyId1) -> bool {
    ...
    Sequencer<SequencerTestData::SequenceKey, Hasher, KeyEqual> sequencer(getDispatcher(), config);
...
TEST_P(SequencerTest, PerformanceTest)
{
    ...
    testSequencerPerformance<SequencerTestData::TaskSequencer>(
        "Highly dependent tasks", getDispatcher(), sleepTime, 3, 2, taskCount, 10, 1);

    testSequencerPerformance<SequencerTestData::TaskSequencer>(
        "Independent tasks", getDispatcher(), sleepTime, taskCount, 1, taskCount, 0, 0);
}
*/
